			...
			foreach ( $files as $file ) {
				...
				if ( empty( $pattern_data['slug'] ) ) {
					_doing_it_wrong(
						...
						sprintf(
							...
							__( 'Could not register file "%s" as a block pattern ("Slug" field missing)', 'cozy-essential-addons' ),
							$file
						),
						'6.0.0'
					);
				...
				if ( ! preg_match( '/^[A-z0-9\/_-]+$/', $pattern_data['slug'] ) ) {
					_doing_it_wrong(
						...
						sprintf(
							...
							__( 'Could not register file "%1$s" as a block pattern (invalid slug "%2$s")', 'cozy-essential-addons' ),
							$file,
							$pattern_data['slug']
						),
						'6.0.0'
				...
				if ( ! $pattern_data['title'] ) {
					_doing_it_wrong(
						...
						sprintf(
							/* translators: %1s: file name; %2s: slug value found. */
							__( 'Could not register file "%s" as a block pattern ("Title" field missing)', 'cozy-essential-addons' ),
							$file
						),
						'6.0.0'
					);
